...
void InitReqScheduler()
{
	...
	completeFlagTablePtr = Addr2Mem(COMPLETE_FLAG_TABLE, COMPLETE_FLAG_TABLE_ADDR) /* jy */;
	statusReportTablePtr = Addr2Mem(STATUS_REPORT_TABLE, STATUS_REPORT_TABLE_ADDR); /* jy */
	eccErrorInfoTablePtr = Addr2Mem(ERROR_INFO_TABLE, ERROR_INFO_TABLE_ADDR); /* jy */
	retryLimitTablePtr = Addr2Mem(RETRY_LIMIT_TABLE, RETRY_LIMIT_TABLE_ADDR); /* jy */

	dieStateTablePtr = Addr2Mem(DIE_STATE_TABLE, DIE_STATE_TABLE_ADDR); /* jy */
	wayPriorityTablePtr = Addr2Mem(WAY_PRIORITY_TABLE, WAY_PRIORITY_TABLE_ADDR); /* jy */
...
void IssueNandReq(unsigned int chNo, unsigned int wayNo)
{
	...
	dataBufAddr = Addr2Mem(void, GenerateDataBufAddr(reqSlotTag)); /* jy */
	spareDataBufAddr = Addr2Mem(void, GenerateSpareDataBufAddr(reqSlotTag)); /* jy */
	...
	switch(dieStateTablePtr->dieState[chNo][wayNo].dieState)
	{
		...
		case DIE_STATE_EXE:
			...
			else if(reqStatus == REQ_STATUS_FAIL)
			{
				...
				if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_READ)
					printf("Read Trigger FAIL on      ");
				else if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_READ_TRANSFER)
					printf("Read Transfer FAIL on     ");
				else if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_WRITE)
					printf("Write FAIL on             ");
				else if(reqPoolPtr->reqPool[reqSlotTag].reqCode == REQ_CODE_ERASE)
					printf("Erase FAIL on             ");

				rowAddr = GenerateNandRowAddr(reqSlotTag);
				printf("ch %x way %x rowAddr %x / completion %x statusReport %x \r\n", chNo, wayNo, rowAddr, completeFlagTablePtr->completeFlag[chNo][wayNo],statusReportTablePtr->statusReport[chNo][wayNo]);
				...
				if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.nandEcc == REQ_OPT_NAND_ECC_OFF)
					if(reqPoolPtr->reqPool[reqSlotTag].reqOpt.dataBufFormat == REQ_OPT_DATA_BUF_ADDR)
					{
						...
						badCheck = Addr2Mem(unsigned char, reqPoolPtr->reqPool[reqSlotTag].dataBufInfo.addr); /* jy */
			...
			{
				rowAddr = GenerateNandRowAddr(reqSlotTag);
				printf("ECC Uncorrectable Soon on ch %x way %x rowAddr %x / completion %x statusReport %x \r\n", chNo, wayNo, rowAddr, completeFlagTablePtr->completeFlag[chNo][wayNo],statusReportTablePtr->statusReport[chNo][wayNo]);
